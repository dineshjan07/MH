...
public class UNB_RET_BUSINESS_DT_INQ_REQOUT extends UserRule{
	private static Logger log = LoggerFactory
	.getLogger(UNB_RET_BUSINESS_DT_INQ_REQOUT.class);

public int Process(HashMap orderedhashtable, String pIPMsg, String s,
	HashMap orderedhashtable1, ErrorHandler erHandler) {

log.debug("Inside UNB_RET_BUSINESS_DT_INQ_REQOUT  --->>>" + pIPMsg);
...
if (pIPMsg == null) {
	...
	try {
		...
		inputStr = inputStr.replaceAll("<xml><RET_BUSINESS_DT_INQ_REQ_COMP_OUT>","");
		inputStr = inputStr.replaceAll("</RET_BUSINESS_DT_INQ_REQ_COMP_OUT></xml>","");
		
		
		String[] inputDetails={"<T24WSRequestparams_Out>","</T24WSRequestparams_Out>","<RET_BUSINESS_DT_INQ_REQ_BODY_OUT>","</RET_BUSINESS_DT_INQ_REQ_BODY_OUT>"};
	...
	//	StringBuffer headerendtag = new StringBuffer("</soap:Envelope>");
		...
		log.debug("Response Message from the UNB_RET_BUSINESS_DT_INQ_REQOUT  is ==>"
				+ _cresultMessage[0]);
		return _iresultCode;
	} catch (Exception e) {
		_iresultCode = 2;
		_cresultMessage[0] = "Exception in In_Data_UserExit UNB_RET_BUSINESS_DT_INQ_REQOUT"
				+ e;
		log.error("Exception in UNB_RET_BUSINESS_DT_INQ_REQOUT : ", e);
		return _iresultCode;
	}
}
}

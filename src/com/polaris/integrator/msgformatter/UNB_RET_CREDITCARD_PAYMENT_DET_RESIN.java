...
public class UNB_RET_CREDITCARD_PAYMENT_DET_RESIN extends UserRule{
	private static Logger log = LoggerFactory.getLogger( UNB_RET_CREDITCARD_PAYMENT_DET_RESIN.class );
	public int Process(HashMap orderedhashtable, String pIPMsg, String s, HashMap orderedhashtable1,ErrorHandler erHandler){
		
		log.debug( "Inside UNB_RET_CREDITCARD_PAYMENT_DET_RESIN  --->>>" + pIPMsg );
		...
		if (pIPMsg==null){
			...
			try{
				...
				inputMsg=inputMsg.replaceAll("&gt;",">");
				inputMsg = inputMsg.replaceAll("<ns:processCreditCardPaymentResponse xmlns:ns=\"http://ws.apache.org/axis2\">", "");
				inputMsg = inputMsg.replaceAll("</ns:processCreditCardPaymentResponse>", "");
				 ...
				 * 
				 */
				String[] inputDetails={"<processCreditCardPaymentResponse>","</processCreditCardPaymentResponse>","<processCreditCardPaymentOutput>","</processCreditCardPaymentOutput>","<T24WSResponseParams>","</T24WSResponseParams>","<processCreditCardPaymentDetails>","</processCreditCardPaymentDetails>","<Body>","</Body>","<Envelope>","<Header/>"};
				String[] outputDetails={"<processCreditCardPaymentResponse_IN>","</processCreditCardPaymentResponse_IN>","<processCreditCardPaymentOutput_IN>","</processCreditCardPaymentOutput_IN>","<T24WSResponseParams_IN>","</T24WSResponseParams_IN>","<processCreditCardPaymentDetails_IN>","</processCreditCardPaymentDetails_IN>","<RET_CREDITCARD_PAYMENT_DET_RES_BODY_IN>","</RET_CREDITCARD_PAYMENT_DET_RES_BODY_IN>","",""};
				for(int i=0;i<inputDetails.length;i++){
				...
				_iresultCode=0;
				_cresultMessage[0]="<RET_CREDITCARD_PAYMENT_DET_RES_COMP_IN>"+coutputStr.toString()+"</RET_CREDITCARD_PAYMENT_DET_RES_COMP_IN>";

				log.debug("Response Message from the UNB_RET_CREDITCARD_PAYMENT_DET_RESIN  is ==>"+ _cresultMessage[0]);
				return _iresultCode;
			}catch(Exception e){
				_iresultCode = 2;
				_cresultMessage[0]="Exception in UNB_RET_CREDITCARD_PAYMENT_DET_RESIN user exit "+e;
				log.error( "Exception in UNB_RET_CREDITCARD_PAYMENT_DET_RESIN : " , e );
				return _iresultCode;
			}
		}
	}

...
public class UNB_RET_RELATIONSHIP_LIST_RESIN extends UserRule{
	private static Logger log = LoggerFactory.getLogger( UNB_RET_RELATIONSHIP_LIST_RESIN.class );
	public int Process(HashMap orderedhashtable, String pIPMsg, String s, HashMap orderedhashtable1,ErrorHandler erHandler){
		
		log.debug( "Inside UNB_RET_RELATIONSHIP_LIST_RESIN  --->>>" + pIPMsg );
		...
		if (pIPMsg==null){
			...
			try{
				...
				inputMsg=inputMsg.replaceAll("&gt;",">");
				inputMsg = inputMsg.replaceAll("<ns:getRelationshipListResponse xmlns:ns=\"http://ws.apache.org/axis2\">", "");
				inputMsg = inputMsg.replaceAll("</ns:getRelationshipListResponse>", "");
				 ...
				 * 
				 */
				String[] inputDetails={"<getRelationshipListResponse>","</getRelationshipListResponse>","<getRelationshipListOutput>","</getRelationshipListOutput>","<T24WSResponseParams>","</T24WSResponseParams>","<RelationshipListDetails>","</RelationshipListDetails>","<Body>","</Body>","<Envelope>","<Header/>"};
				String[] outputDetails={"<getRelationshipListResponse_IN>","</getRelationshipListResponse_IN>","<getRelationshipListOutput_IN>","</getRelationshipListOutput_IN>","<T24WSResponseParams_IN>","</T24WSResponseParams_IN>","<RelationshipListDetails_IN>","</RelationshipListDetails_IN>","<RET_RELATIONSHIP_LIST_RES_BODY_IN>","</RET_RELATIONSHIP_LIST_RES_BODY_IN>","",""};
				for(int i=0;i<inputDetails.length;i++){
				...
				_iresultCode=0;
				_cresultMessage[0]="<RET_RELATIONSHIP_LIST_RES_COMP_IN>"+coutputStr.toString()+"</RET_RELATIONSHIP_LIST_RES_COMP_IN>";

				log.debug("Response Message from the UNB_RET_RELATIONSHIP_LIST_RESIN  is ==>"+ _cresultMessage[0]);
				return _iresultCode;
			}catch(Exception e){
				_iresultCode = 2;
				_cresultMessage[0]="Exception in UNB_RET_RELATIONSHIP_LIST_RESIN user exit "+e;
				log.error( "Exception in UNB_RET_RELATIONSHIP_LIST_RESIN : " , e );
				return _iresultCode;
			}
		}
	}

...
public class UNB_RET_BILL_INQ_RESIN extends UserRule{
	private static Logger log = LoggerFactory.getLogger( UNB_RET_BILL_INQ_RESIN.class );
	public int Process(HashMap orderedhashtable, String pIPMsg, String s, HashMap orderedhashtable1,ErrorHandler erHandler){
		
		log.debug( "Inside UNB_RET_BILL_INQ_RESIN  --->>>" + pIPMsg );
		...
		if (pIPMsg==null){
			...
			try{
				...
				inputMsg=inputMsg.replaceAll("&gt;",">");
				inputMsg = inputMsg.replaceAll("<ns:BillInqResponse xmlns:ns=\"http://ws.apache.org/axis2\">", "");
				inputMsg = inputMsg.replaceAll("</ns:BillInqResponse>", "");
				 ...
				 * 
				 */
				String[] inputDetails={"<BalInqResponse>","</BalInqResponse>","<Reply>","</Reply>","<BalInqResult>","</BalInqResult>","<Body>","</Body>","<Envelope>","<Header/>"};
				String[] outputDetails={"<BalInqResponse_IN>","</BalInqResponse_IN>","<Reply_IN>","</Reply_IN>","<BalInqResult_IN>","</BalInqResult_IN>","<RET_BILL_INQ_RES_BODY_IN>","</RET_BILL_INQ_RES_BODY_IN>","",""};
				for(int i=0;i<inputDetails.length;i++){
				...
				_iresultCode=0;
				_cresultMessage[0]="<RET_BILL_INQ_RES_COMP_IN>"+coutputStr.toString()+"</RET_BILL_INQ_RES_COMP_IN>";

				log.debug("Response Message from the UNB_RET_BILL_INQ_RESIN  is ==>"+ _cresultMessage[0]);
				return _iresultCode;
			}catch(Exception e){
				_iresultCode = 2;
				_cresultMessage[0]="Exception in UNB_RET_BILL_INQ_RESIN user exit "+e;
				log.error( "Exception in UNB_RET_BILL_INQ_RESIN : " , e );
				return _iresultCode;
			}
		}
	}

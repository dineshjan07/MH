...
public class UNB_ACC_STMT_REQOUT extends UserRule{
	private static Logger log = LoggerFactory
	.getLogger(UNB_ACC_STMT_REQOUT.class);

public int Process(HashMap orderedhashtable, String pIPMsg, String s,
	HashMap orderedhashtable1, ErrorHandler erHandler) {

log.debug("Inside UNB_ACC_STMT_REQOUT  --->>>" + pIPMsg);
...
if (pIPMsg == null) {
	...
	try {
		...
		inputStr = inputStr.replaceAll("<xml><ACC_STMT_REQ_COMP_OUT>",
				"");
		inputStr = inputStr.replaceAll(
				"</ACC_STMT_REQ_COMP_OUT></xml>", "");
		...
		*/
		...
		log.debug("Response Message from the UNB_ACC_STMT_REQOUT  is ==>"
				+ _cresultMessage[0]);
		return _iresultCode;
	} catch (Exception e) {
		_iresultCode = 2;
		_cresultMessage[0] = "Exception in In_Data_UserExit UNB_ACC_STMT_REQOUT"
				+ e;
		log.error("Exception in UNB_ACC_STMT_REQOUT : ", e);
		return _iresultCode;
	}
}
}

...
public class UNB_ACC_SUMM_REQOUT extends UserRule {

	private static Logger log = LoggerFactory
			.getLogger(UNB_ACC_SUMM_REQOUT.class);

	public int Process(HashMap orderedhashtable, String pIPMsg, String s,
			HashMap orderedhashtable1, ErrorHandler erHandler) {

		log.debug("Inside UNB_ACC_SUMM_REQOUT  --->>>" + pIPMsg);
		...
		if (pIPMsg == null) {
			...
			try {
				...
				*/
				...
				log.debug("Response Message from the UNB_ACC_SUMM_REQOUT  is ==>"
						+ _cresultMessage[0]);
				return _iresultCode;
			} catch (Exception e) {
				_iresultCode = 2;
				_cresultMessage[0] = "Exception in In_Data_UserExit UNB_ACC_SUMM_REQOUT"
						+ e;
				log.error("Exception in UNB_ACC_SUMM_REQOUT : ", e);
				return _iresultCode;
			}
		}
	}

...
public class UNB_DET_SUMM_RESIN extends UserRule{
	private static Logger log = LoggerFactory.getLogger( UNB_DET_SUMM_RESIN.class );
	public int Process(HashMap orderedhashtable, String pIPMsg, String s, HashMap orderedhashtable1,ErrorHandler erHandler){

		log.debug( "Inside UNB_DET_SUMM_RESIN1  --->>>" + pIPMsg );
		...
		if (pIPMsg==null){
			...
			try{
				...
				_iresultCode=0;
				_cresultMessage[0]="<ACC_DET_RES_COMP_IN>"+coutputStr.toString()+"</ACC_DET_RES_COMP_IN>";

				log.debug("Response Message from the UNB_ACC_DET_RESIN  is ==>"+ _cresultMessage[0]);
				return _iresultCode;
			}catch(Exception e){
				_iresultCode = 2;
				_cresultMessage[0]="Exception in UNB_DET_SUMM_RESIN user exit "+e;
				log.error( "Exception in UNB_DET_SUMM_RESIN1 : " , e );
				return _iresultCode;
			}
		}
	}

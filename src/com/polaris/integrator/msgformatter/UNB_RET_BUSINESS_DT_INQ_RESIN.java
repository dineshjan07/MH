...
public class UNB_RET_BUSINESS_DT_INQ_RESIN extends UserRule {
		private static Logger log = LoggerFactory.getLogger( UNB_RET_BUSINESS_DT_INQ_RESIN.class );
	public int Process(HashMap orderedhashtable, String pIPMsg, String s, HashMap orderedhashtable1,ErrorHandler erHandler){
		
		log.debug( "Inside UNB_RET_BUSINESS_DT_INQ_RESIN  --->>>" + pIPMsg );
		...
		if (pIPMsg==null){
			...
			try{
				...
				inputMsg=inputMsg.replaceAll("&gt;",">");
				inputMsg = inputMsg.replaceAll("<ns:getBusinessDateResponse xmlns:ns=\"http://ws.apache.org/axis2\">", "");
				inputMsg = inputMsg.replaceAll("</ns:getBusinessDateResponse>", "");
				 ...
				 * 
				 */
				String[] inputDetails={"<getDateResponse>","</getDateResponse>","<getDateOutput>","</getDateOutput>","<DateDetails>","</DateDetails>","<T24WSResponseParams>","</T24WSResponseParams>","<Body>","</Body>","<Envelope>","<Header/>"};
				String[] outputDetails={"<getDateResponse_IN>","</getDateResponse_IN>","<getDateOutput_IN>","</getDateOutput_IN>","<DateDetails_IN>","</DateDetails_IN>","<T24WSResponseParams_RETBD_IN>","</T24WSResponseParams_RETBD_IN>","<RET_BUSINESS_DT_INQ_RES_BODY_IN>","</RET_BUSINESS_DT_INQ_RES_BODY_IN>","",""};
				for(int i=0;i<inputDetails.length;i++){
				...
				_iresultCode=0;
				_cresultMessage[0]="<RET_BUSINESS_DT_INQ_RES_COMP_IN>"+coutputStr.toString()+"</RET_BUSINESS_DT_INQ_RES_COMP_IN>";

				log.debug("Response Message from the UNB_RET_BUSINESS_DT_INQ_RESIN  is ==>"+ _cresultMessage[0]);
				return _iresultCode;
			}catch(Exception e){
				_iresultCode = 2;
				_cresultMessage[0]="Exception in UNB_RET_BUSINESS_DT_INQ_RESIN user exit "+e;
				log.error( "Exception in UNB_RET_BUSINESS_DT_INQ_RESIN : " , e );
				return _iresultCode;
			}
		}
	}

...
public class UNB_ACC_SUMM_RESIN extends UserRule{
	...
	public int Process(HashMap orderedhashtable, String pIPMsg, String s, HashMap orderedhashtable1,ErrorHandler erHandler){
		...
		log.debug( "Inside UNB_ACC_SUMM_RESIN  --->>>" + pIPMsg );
		...
		if (pIPMsg==null){
			...
			try{
				...
				String[] outputDetails={"<ACC_SUMM_RES_BODY_IN>","</ACC_SUMM_RES_BODY_IN>"};
				for(int i=0;i<inputDetails.length;i++){
				...
				log.debug("Response Message from the UNB_ACC_SUMM_RESIN  is ==>"+ _cresultMessage[0]);
				return _iresultCode;
			}catch(Exception e){
				_iresultCode = 2;
				_cresultMessage[0]="Exception in UNB_ACC_SUMM_RESIN user exit "+e;
				log.error( "Exception in UNB_ACC_SUMM_RESIN : " , e );
				return _iresultCode;
			}
		}
	}

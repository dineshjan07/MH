...
public class UNB_ACC_SUMM_REQOUT1 extends UserRule{
	private static Logger log = LoggerFactory
	.getLogger(UNB_ACC_SUMM_REQOUT1.class);

public int Process(HashMap orderedhashtable, String pIPMsg, String s,
	HashMap orderedhashtable1, ErrorHandler erHandler) {

log.debug("Inside UNB_ACC_SUMM1_REQOUT  --->>>" + pIPMsg);
...
if (pIPMsg == null) {
	...
	try {
		...
		inputStr = inputStr.replaceAll("<xml><ACC_SUMM_REQ_COMP_OUT>","");
		inputStr = inputStr.replaceAll("<ACC_SUMM_REQ_BODY_OUT>","");
		inputStr = inputStr.replaceAll("</ACC_SUMM_REQ_BODY_OUT>","");
		inputStr = inputStr.replaceAll("</ACC_SUMM_REQ_COMP_OUT></xml>","");
	...
	//	StringBuffer headerendtag = new StringBuffer("</soap:Envelope>");
		...
		log.debug("Response Message from the UNB_ACC_SUMM1_REQOUT  is ==>"
				+ _cresultMessage[0]);
		return _iresultCode;
	} catch (Exception e) {
		_iresultCode = 2;
		_cresultMessage[0] = "Exception in In_Data_UserExit UNB_ACC_SUMM1_REQOUT"
				+ e;
		log.error("Exception in UNB_ACC_SUMM1_REQOUT : ", e);
		return _iresultCode;
	}
}
}
